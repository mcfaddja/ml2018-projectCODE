...
nEST = int(sys.argv[2])
bagIN = BaggingClassifier(n_jobs=nJOBS, n_estimators=nEST)

#bagIN.fit(likesMAT, sexsARR)
bagIN.fit(X_train, y_train)

print("sexs, bagIN:  ", str(nEST), " ", bagIN.score(X_test, y_test))

# joblib.dump(bagIN, "/Users/jamster/bagIN-A-sexs.xz", compress=9)

# impbagIN = joblib.load("/Users/jamster/bagIN-A-sexs.xz")